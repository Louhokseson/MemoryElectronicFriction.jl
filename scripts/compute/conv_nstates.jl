...
nstates_list = [2^i for i in 2:6]
params_list = dict_list(Dict{String, Any}(
    "width" => [4],
    "discretisation" => [NQCModels.ShenviGaussLegendre, NQCModels.TrapezoidalRule],
...
for (i,params_dict) in enumerate(params_list)
    ...
    path = mkpath(datadir("sims", "lambda", string(params_dict["impuritymodel"]), string(nameof(params_dict["discretisation"])), "energy="*string(params_dict["energy"]) ) )

    name = savename(params_dict; allowedtypes=(Number, String, Symbol, UnionAll)) * ".txt"
    for nstates in nstates_list

        params_dict["nstates"] = nstates

        bath, adsorbate_m, position_au, energy_au, temperature_au = buildSystemBath(params_dict)

        @info "Position $(params_dict["position"]) Å --- Temperature $(params_dict["temperature"]) K --- nstates = $(nstates) --- Discretisation = $(string(nameof(params_dict["discretisation"])))"
        ...
        push!(lambda_nstates, Lambda_au)
    end

    header = ["nstates" "Lambda_au"]

    full_data = vcat(header, hcat(nstates_list, lambda_nstates))
