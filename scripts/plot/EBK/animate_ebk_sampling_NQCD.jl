# =============================================================================
# EBK initial-vibration sampling, animated — MCMC variant.
# Gardner–Habershon–Maurer NO/Au(111) (2-D POGO model used in run_md.jl).
#
# Same two-step story as animate_ebk_sampling.jl, but STEP 2 mirrors the
# PRODUCTION sampler (NQCDynamics QuantisedDiatomic.select_random_bond_lengths):
#
#   STEP 1 (quantise):  integer ν → bond energy Eν via EBK ∮p dr = 2π(ν+½)ℏ.
#
#   STEP 2 (sample, MCMC):  bond lengths are NOT taken from a time-propagated
#       orbit. They are drawn by Metropolis–Hastings with a UNIFORM proposal on
#       [r₁,r₂] targeting  P(r) ∝ 1/|ṙ(r)| = 1/√(2μ(Eν−V(r))).  The speed is then
#       |ṙ| = √(2(Eν−V)/μ) with a RANDOM ± sign. So every draw lands on the
#       energy shell, but the marker JUMPS over it (random walk) rather than
#       gliding. The resulting (r, ṙ) ensemble is identical to the time-average.
#
# POGO binding curve re-implemented in atomic units (verified vs build_V_eff:
# V_min=-0.1035 eV, r_eq=1.152 Å, E₁₆=3.238 eV) → CairoMakie only, no NQCD load.
#
# Output: docs/ebk_sampling_nu16_NQCD.gif
# =============================================================================
...
# phase-space loop p=±√(2μ(E−V)) → (r[Å], ṙ[Å/fs]); closed (the "energy shell")
function loop_pts(E)
...
# ---------------------------------------- Metropolis–Hastings chain at fixed Eν
# Independence sampler: uniform proposal on (r₁,r₂), target P(r) ∝ 1/p(r).
# log π(r) = −log p(r) = −½ log(2μ(Eν−V(r))).  Sign of ṙ is a fresh coin flip
# per recorded draw (exactly as select_random_bond_lengths builds `momenta`).
pmom(r) = sqrt(max(2μ*(E16 - Vbond(r)), 0.0))     # |p|(r)
logπ(r) = -0.5*log(2μ*(E16 - Vbond(r)))
function mcmc_chain(n)
    Random.seed!(20240624)
    cr = Vector{Float64}(undef, n); cv = similar(cr)
    cur = r_eq
    for i in 1:n
        prop = r1au + rand()*(r2au - r1au)
        if log(rand()) < (logπ(prop) - logπ(cur))
            cur = prop
        end
        cr[i] = cur
        cv[i] = (pmom(cur)/μ) * rand((-1, 1))      # signed radial velocity (a.u.)
    end
    return cr, cv
end
const NCHAIN = 8000
const CR, CV = mcmc_chain(NCHAIN)                  # bond lengths (a.u.), velocities (a.u.)
const CKE = @. 0.5 * μ * CV^2 * Ha2eV              # kinetic energy, eV
...
# limiting distributions (from the FULL chain) → fixes y-limits + target curve
const r_final  = hcounts(CR .* Bohr2Å, r_edges)
const ke_final = hcounts(CKE,          ke_edges)
const r_ymax   = maximum(r_final) * 1.18
const ke_ymax  = maximum(ke_final) * 1.18
# analytic target density ρ(r) ∝ 1/|v(r)|, scaled to the final histogram
...
ax_ph  = Axis(fig[1:2, 2]; xlabel = "r / Å", ylabel = "ṙ / (Å fs⁻¹)",
              title = "phase space  (r, ṙ): Metropolis draws on the energy shell", titlesize = 17, AXKW...)
ax_hr  = Axis(fig[1, 3]; xlabel = "sampled r / Å", ylabel = "count",
              title = "STEP 2a: bond lengths", titlesize = 18, AXKW...)
...
# dynamic observables
o_ladder = Observable(Point2f[])
o_elevel = Observable(Point2f[Point2f(r1Å,Vmin_eV), Point2f(r2Å,Vmin_eV)])
o_tp     = Observable(Point2f[])
o_ball   = Observable(Point2f[])
o_loop   = Observable(loop_pts(Eladder[1]))
o_pmark  = Observable(Point2f[])
o_psamp  = Observable(Point2f[])
o_trail  = Observable(Point2f[])        # last few MCMC draws (chain order) → shows jumping
...
scatter!(ax_ph, o_psamp; color = (col.samp,0.55), markersize = 6)
lines!(ax_ph, o_trail; color = (col.ball,0.5), linewidth = 1.2)
...
text!(ax_hr, r_edges[1]+0.02, r_ymax*0.97; text = "dashed: P(r) ∝ 1/|ṙ|",
...
# Storyboard
# =============================================================================
const N1 = 120   # quantise (build ladder)
const N2 = 22    # hold ν = 16
const N3 = 130   # sample (MCMC) — a touch longer so the jumping reads clearly
const NF = N1 + N2 + N3
...
const CAP3 = "STEP 2 — SAMPLE (Metropolis).  Draw r from P(r) ∝ 1/|ṙ| by a random walk; set |ṙ| = √(2(Eν−V)/μ) with a random ± sign.\nDraws JUMP over the energy shell (not gliding);  r piles at the turning points;  KE = ½μṙ² is U-shaped (mostly ≈ 0, max = Eν − V_min)."

gifpath = projectdir("docs", "ebk_sampling_nu16_NQCD.gif")
...
function update!(f)
    if f <= N1                                   # ---- ACT 1: quantise
        ...
        o_tp[]     = tp_segs(E)
        o_loop[]   = loop_pts(E)
        o_ball[]   = Point2f[]; o_pmark[] = Point2f[]; o_psamp[] = Point2f[]; o_trail[] = Point2f[]
    ...
    elseif f <= N1 + N2                          # ---- ACT 2: hold ν = 16
        ...
        o_hr[] = hcounts(CR[1:m] .* Bohr2Å, r_edges)
        o_ke[] = hcounts(CKE[1:m],          ke_edges)
        o_read[] = @sprintf("STEP 2 · Metropolis sampling ν = 16   ·   %d draws", m)
    end
end
...
if haskey(ENV, "EBK_STILLDIR")
    for (n, fr) in [("act1", 60), ("act2", N1 + 10), ("act3", NF)]
        update!(fr)
        save(joinpath(ENV["EBK_STILLDIR"], "ebk_NQCD_still_$(n).png"), fig)
    end
end
