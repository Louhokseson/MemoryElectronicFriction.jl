...
# Unit conversion: atomic friction → u⋅ps⁻¹
unit_conv = ustrip(auconvert(u"u", 1) / auconvert(u"ps", 1))

# Font
const MINION_FONT = projectdir("fonts", "MinionPro-Capt.otf")

# Figure geometry
# The figure is authored at the full two-column width (figure*) so it embeds at
# scale 1.0 — 1 Makie pt = 1 typeset pt — keeping fonts and line widths exactly
# as set here. Landscape (height ≈ 0.55·width) so the eight panels read WIDE.
...
function threshold_energy(params_dict::Dict{String, Any})
    ...
    return sqrt(sqrt(6) * abs(∇h / ∇Δ * Δ - h)^2 + (2π^2 * temperature_au^2))
end
...
legend_kwargs = (
    ...
    colgap        = scaled(5),
    groupgap      = scaled(0),    # group separation comes ENTIRELY from the spaces around the "|" titles below
    titlegap      = scaled(4),
