...
function plot_vib_state_distribution_compare(ν_INITIAL_LIST, E_TRANS_eV_LIST,
                                              ...
                                              T_focus          = 300,
                                              ν_max            = 20,
                                              # AIP double-column width (≈ 6.69 in / 170 mm).
                                              # Height ratio 0.7 → ~4.7 in; matches plot_vib_DeltaE_kernel.jl.
                                              figsize          = (HokseonPlots.TWO_COLUMN_WIDTH,
                                                                  HokseonPlots.TWO_COLUMN_WIDTH),
                                              panel_spinewidth = 1.8,
                                              show_md_baseline = false,
                                              ylims            = (-0.05, nothing),
                                              # Font hierarchy (pt at the authored full-width size).
                                              title_fontsize   = 20,   # panel Eₜ headers
                                              label_fontsize   = 20,   # axis labels (Probability / Final vibrational state ν)
                                              tick_fontsize    = 18,   # x / y tick labels
                                              legend_fontsize  = 18,
                                              panel_label_fontsize = 18)  # (a)–(f) tags
    ...
    for (r_idx, ν_init) in enumerate(ν_INITIAL_LIST)
        ...
        for (c_idx, Eₜ) in enumerate(E_TRANS_eV_LIST)
            ...
            nu_bins_vals = collect(0:ν_max)
            memory_prob = Float64[]
            memory_err  = Float64[]
            markovian_prob = Float64[]
            markovian_err  = Float64[]
            md_prob = Float64[]
            md_err  = Float64[]
            has_md  = false
            ...
            for (res, col, mrk) in (
                                    #(res_arith, color_arith, :utriangle),
            ...
            panel_letter_str = "($(panel_letter))"
            push!(panel_data, (;
                row = r_idx, col = c_idx,
                nu_init = ν_init, E_trans_eV = Eₜ,
                panel_letter = panel_letter_str,
                nu_bins = nu_bins_vals,
                memory_prob = memory_prob, memory_err = memory_err,
                markovian_prob = markovian_prob, markovian_err = markovian_err,
                md_prob = md_prob, md_err = md_err,
                has_md = has_md,
            ))
        end
    end
...
configs_noau = vcat(memory_endpt_configs, markovian_configs)

result = plot_vib_state_distribution_compare(ν_INITIAL_LIST, E_TRANS_eV_LIST, configs_noau;
...
save(plotsdir("cpa", "NOAu", "vib_state_distribution.pdf"), fig)

# Save figure data to HDF5 for reproduction
figure_data_dir = projectdir("figure_data", "fig_5")
mkpath(figure_data_dir)
h5_path = joinpath(figure_data_dir, "fig_5_data.h5")

h5open(h5_path, "w") do h5
    h5["nu_initial_list"] = collect(Float64, ν_INITIAL_LIST)
    ...
    for pd in panel_data
        ...
        panel = create_group(h5, panel_name)
        panel["nu_init"] = Float64(pd.nu_init)
        panel["E_trans_eV"] = Float64(pd.E_trans_eV)
        panel["panel_letter"] = pd.panel_letter
        panel["nu_bins"] = collect(Float64, pd.nu_bins)
        panel["nu_init_marker"] = Float64(pd.nu_init)

        memory_g = create_group(panel, "memory")
        memory_g["prob"] = collect(Float64, pd.memory_prob)
        memory_g["err"]  = collect(Float64, pd.memory_err)

        markov_g = create_group(panel, "markovian")
        markov_g["prob"] = collect(Float64, pd.markovian_prob)
        markov_g["err"]  = collect(Float64, pd.markovian_err)

        if pd.has_md
            md_g = create_group(panel, "md")
            md_g["prob"] = collect(Float64, pd.md_prob)
            md_g["err"]  = collect(Float64, pd.md_err)
        end
    end
end
