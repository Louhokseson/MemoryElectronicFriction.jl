...
function CPA_dict_to_data_savename(p::Dict{String,Any}, cfg::Dict{String,Any})
    ...
    # Memory CPA carries `kernel_average`; Markovian CPA does not. Use that as
    # the variant discriminator so the two land in disjoint folders.
    variant    = haskey(cfg, "kernel_average") ? "memory" : "markovian"
