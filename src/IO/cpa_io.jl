...
function CPA_dict_to_data_savename(p::Dict{String,Any}, cfg::Dict{String,Any})
    ...
    # Variant discriminator → folder name. An explicit `"variant"` key wins
    # (used e.g. by the memory→Markovian zero-frequency test, which lands in
    # …/<model>/test). Otherwise auto-detect: memory CPA carries
    # `kernel_average`, Markovian CPA does not.
    variant    = String(get(cfg, "variant",
                            haskey(cfg, "kernel_average") ? "memory" : "markovian"))
    savingpath = joinpath("sims", "cpa", String(cfg["model"]), variant)
